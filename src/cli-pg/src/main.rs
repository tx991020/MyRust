...
fn delete(conn: &Connection) -> Result<Vec<(String, String)>, Error> {
    let res = conn.query("SELECT name, email FROM users", &[])?.into_iter()
        .map(|row| (row.get(0), row.get(1)))
        .collect();
    Ok(res)
...
fn main() -> Result<(), Error> {
    let matches = App::new(crate_name!())
        ...
        .subcommand(SubCommand::with_name(CMD_DELETE).about("delete")
    ...
    let addr = matches.value_of("database")
        .unwrap_or("postgres://postgres:123456@localhost:5432");
